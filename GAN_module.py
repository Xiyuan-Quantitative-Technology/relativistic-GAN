...
class Discriminator(nn.Module):
    ...
    def forward(self, input):
        regress_val = self.discriminator_layer(input)
        return regress_val
    
    
    
class Generator(nn.Module):
    def __init__(self, lat_v_D, gen_hid_D):
        ...
        self.generator_layer = nn.Sequential(*block(lat_v_D, gen_hid_D),
                                               # *block(gen_hid_D, gen_hid_D),
                                                # *block(gen_hid_D, gen_hid_D),
                                                # *block(gen_hid_D, gen_hid_D),
                                                # *block(gen_hid_D, gen_hid_D),
                                             nn.Linear(gen_hid_D, lat_v_D))
                                             
                                             
    def forward(self, ran_input):
        virtual_latspace = self.generator_layer(ran_input)
        return virtual_latspace
        
        
        
#####################################################################
class GAN(nn.Module):
    def __init__(self, vocab, emb_D, encod_hidden_D, decod_hidden_D, \
        ...
        # Generator
        self.generator = Generator(lat_v_D, gen_hid_D)
    ...
    def generator_forward(self, ran_in):
        return self.generator.forward(ran_in)
    ...
    def samples_generation(self, No_sam, max_len=100):
        with torch.no_grad():
            ...
            ran_z = self.generator_forward(ran_z)
